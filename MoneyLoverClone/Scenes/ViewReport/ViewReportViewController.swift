    ...
    // MARK: - Data
       private func setupData() {
            ...
            tableView.do {
               ...
               $0.register(cellType: MoneyTableViewCell.self)
               $0.register(cellType: PieChartTableViewCell.self)
            }
...
extension ViewReportViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "Cell", for: indexPath) as? CollectionViewCell ?? nil
        let year = date.year
...
extension ViewReportViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if section == 0 {
            let cell: MoneyTableViewCell = tableView.dequeueReusableCell(for: indexPath)
            return cell
        } else {
            let cell: PieChartTableViewCell = tableView.dequeueReusableCell(for: indexPath)
            cell.delegate = self
...
extension ViewReportViewController: CellDelegate {
    func showExpense(tag: Int) {
        ...
        let transactionType = TransactionType.expendsed.rawValue
        UserDefaults.standard.set(expense, forKey: "key")
        UserDefaults.standard.set(transactionType, forKey: "transactionType")
        let story = UIStoryboard(name: "Report", bundle: nil)
        guard let viewController = story.instantiateViewController(identifier: "detailReport") as? DetailReportViewController else {
            print("err")
            return
        }
    ...
    func showIncome(tag: Int) {
        ...
        let transactionType = TransactionType.income.rawValue
