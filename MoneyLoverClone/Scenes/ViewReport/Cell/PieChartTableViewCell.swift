        ...
        for index in 0 ..< listTransaction.count {
            if listTransaction[index].date.month == setMonth {
                if listTransaction[index].type == "expendsed" {
                    ...
                    for indexPath in 0..<listCategory.count {
                        if listTransaction[index].categoryID == listCategory[indexPath].identify {
                            entriesExpense.append(PieChartDataEntry(value: Double(listTransaction[index].money), label: listCategory[indexPath].name))
                        }
                    }
                    ...
                    for indexPath in 0..<listCategory.count {
                        if listTransaction[index].categoryID == listCategory[indexPath].identify {
                            entriesIncome.append(PieChartDataEntry(value: Double(listTransaction[index].money), label: listCategory[indexPath].name))
                        }
                    }
